...
def test_mean_piecewise_alignment():
    pwa = PieceWiseAlignment(n_pieces=n_labels, mask=mask_img, standardize=False, detrend=False, method="mean")
    pwa.fit(img1, img2)
...
def test_piecewise_alignment():
    for method in [RidgeCV(alphas=[1e-12]), RRRCV(ks=[10], alphas=[1e-12])]:
        print(method)
        pwa = PieceWiseAlignment(n_pieces=n_labels, mask=mask_img, standardize=False, detrend=False, method=method)
        pwa.fit(img1, img2)
...
def test_piecewise_alignment_parallel2():
    for method in [RidgeCV(alphas=[1e-12]), RRRCV(ks=[10], alphas=[1e-12])]:
        print(method)
        pwa = PieceWiseAlignment(n_pieces=n_labels, mask=mask_img, standardize=False, detrend=False, method=method, n_jobs=2)
        pwa.fit(img1, img2)
        img2_ = pwa.transform(img1)
        assert_array_almost_equal(img2_.get_data(), img2.get_data(), 6)
...
def test_multiple_piecewise_alignment_shapes():
    for method in ["mean", RidgeCV(alphas=[1e-12]), RRRCV(ks=[10], alphas=[1e-12])]:
        print(method)
        img1_m = [img1, img1]
        img2_m = [img2, img2]
        pwa = PieceWiseAlignment(n_pieces=n_labels, mask=mask_img, standardize=False, detrend=False, method=method)
